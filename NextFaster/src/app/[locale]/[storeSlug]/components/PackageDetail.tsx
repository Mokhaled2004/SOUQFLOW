      ...
      <div className={`grid gap-10 overflow-hidden rounded-[2.5rem] bg-white p-6 shadow-2xl shadow-emerald-900/5 ring-1 ring-neutral-100 sm:p-10 lg:grid-cols-2 ${isAr ? 'direction-rtl' : ''}`}>
        ...
        <div className={`flex flex-col justify-center gap-6 ${isAr ? 'lg:order-1 text-right' : ''}`}>
          <div>
            ...
            <h1 className="text-4xl font-black tracking-tight text-neutral-900 sm:text-5xl">{pkg.name}</h1>
            <p className="mt-6 text-base font-medium leading-relaxed text-neutral-500">{pkg.description}</p>
          </div>
              ...
              <div className="grid gap-3">
                {pkg.items.map((item, idx) => (
                  <button
                    ...
                    className={`group/item flex items-center gap-3 rounded-2xl bg-neutral-50 p-3 ring-1 ring-neutral-100 transition-all hover:bg-white hover:ring-emerald-200 hover:shadow-lg hover:shadow-emerald-900/5 ${isAr ? 'flex-row-reverse text-right' : 'text-left'}`}
                  >
                    <div className="h-10 w-10 shrink-0 overflow-hidden rounded-xl bg-white ring-1 ring-neutral-100">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="truncate text-xs font-black text-neutral-900 uppercase group-hover/item:text-emerald-600 transition-colors">{item.product.name}</p>
                      <p className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest">
                        {item.quantity} x {isAr ? 'وحدة' : 'Unit'}
                      </p>
                    </div>
                    <CheckCircle2 className="h-4 w-4 text-emerald-500 shrink-0 opacity-40 group-hover/item:opacity-100 transition-opacity" />
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className={`mt-4 flex flex-col gap-4 sm:flex-row ${isAr ? 'sm:flex-row-reverse' : ''}`}>
             <button
              ...
              className="flex flex-1 items-center justify-center gap-3 rounded-2xl bg-emerald-600 px-10 py-5 text-xs font-black uppercase tracking-[0.2em] text-white shadow-2xl shadow-emerald-900/20 transition-all hover:bg-emerald-700 hover:shadow-emerald-900/30 active:scale-95"
            >
              <ShoppingCart className="h-5 w-5" />
              {isAr ? 'إضافة الباقة للسلة' : 'Add Package to cart'}
            </button>
            <div className="flex items-center justify-center gap-2 px-6 py-2 rounded-2xl bg-neutral-50 border border-neutral-100">
